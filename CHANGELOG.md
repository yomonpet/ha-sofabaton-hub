...
### Fixed
- Subscribe to MQTT response topics before requesting initial Hub data, preventing activity-list timeouts after Home Assistant restarts.
- Unsubscribe MQTT callbacks when unloading the integration to prevent duplicate message handling after reloads.
- Fixed diagnostics downloads failing when accessing the stored coordinator.
